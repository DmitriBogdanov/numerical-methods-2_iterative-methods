...
// @return 1 => aproximate solution
// @return 2 => number of iterations
inline std::tuple<DMatrix, unsigned int> seidel_method_convergence_test(const DMatrix &A, const DMatrix &b, const DMatrix &preciseSol, double epsilon, unsigned int maxIterations) {
	const auto N = A.rows();
	...
	do {
		...
		differenceNorm = 0.;
		for (size_t i = 0; i < N; ++i) differenceNorm = std::max(differenceNorm, std::abs(X(i) - preciseSol(i)));
